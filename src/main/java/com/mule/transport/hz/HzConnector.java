 ...
 * User: shailesh
 * Date: 11/03/2013
 * <code>HzConnector</code>
 * Hazelcast compliant connector that can be used
 * by a Mule endpoint. The connector supports  functionality including Map
 * and queues,modes and local transactions.
 */
public class HzConnector extends AbstractConnector
{
    ...
    public void doInitialise() throws InitialisationException
    {
        ...
        /* IMPLEMENTATION NOTE: Is called once all bean properties have been
           ...
           connectors state. */
        if (hzInstance == null) {
            logger.error("Hazelcast Instance bean not defined in spring xml") ;
            throw new InitialisationException(HzMessages.hazelcastInstanceNotDefined() , this);
        }

        if (logger.isDebugEnabled())
        {
            logger.debug(" Default  Hazelcast Instance for HZ connector:  " + hzInstance.getName());
        }
    }
